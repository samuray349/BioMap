...
function renderUserTable(users, tbody) {
    ...
    users.forEach(user => {
        ...
        // Render suspend icon - show check icon if suspended, clock icon if not
        const suspendIconHtml = isSuspended 
            ? `<i class="fa-solid fa-check suspended-check-icon" data-user-id="${user.utilizador_id}" style="color: #198754; cursor: pointer;" title="Clique para reverter suspensão"></i>`
            : `<i class="fas fa-clock suspend-icon" data-user-id="${user.utilizador_id}" style="cursor: pointer;" title="Suspender utilizador"></i>`;
        
        // Render ban icon - show check icon if banned, ban icon if not
    ...
    tbodyEl.querySelectorAll('.suspended-check-icon').forEach(icon => {
        icon.addEventListener('click', async function(e) {
            e.preventDefault();
            e.stopPropagation();
